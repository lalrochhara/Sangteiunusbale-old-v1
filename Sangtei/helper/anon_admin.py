...
from Sangtei import NICKY_Sangtei_BOT, SangteiCli
from Sangtei.database.chats_settings_mongo import get_anon_setting
from Sangtei.helper.chat_status import anon_admin_checker
...
def anonadmin_checker(func):
    ...
    Args:
        func ([type]): pass client: SangteiCli, message: Message
    """
    async def wrap(client, message: Message):
        ...
        @SangteiCli.on_callback_query(filters.create(lambda _, __, query: 'anonAdminConfirm_' in query.data))
        async def anon_callback(client: SangteiCli, callback_query: CallbackQuery):
