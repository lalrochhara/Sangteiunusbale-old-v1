...
from Sangtei.database.chats_settings_mongo import anonadmin_db, get_anon_setting
...
@SangteiCli.on_message(custom_filter.command(commands=('anonadmin')))
async def anon_admin(client, message):
    ...
    if not await isUserCreator(message):
        await message.reply(
            "Group siamtu chauh in he command hi a hmang thei"
        )
        return
