...
HELP_TEXT = (
    ...
    "» If you encounter any bugs, please feel free to report them in my support chat - @SangteiSupportChat ^^"
)
...
@SangteiCli.on_message(custom_filter.command(commands=('help')))
async def help_command(client, message):
    ...
    if message.chat.type != "private":
        ...
        button_text = 'Tanpui i duh chua hetah hmet rawh!'
        text = 'Tanpui tur che in PM lam ah min rawn be rawh!'
        ...
        if module_name is not None:
            ...
            try:
                ...
                text = f"He `{module_name.capitalize()}` module tana tanpuina chu!" 
                button_text = 'Hmet Rawh!'
...
@SangteiCli.on_message(custom_filter.command('start'), group=9)
async def redirectHelp(client, message):
    if (
        ...
        if len(message.command) >= 2:
            ...
            await help_parser(SangteiCli, message.chat.id, HELP_TEXT)
...
@SangteiCli.on_callback_query(filters.create(help_button_callback))
