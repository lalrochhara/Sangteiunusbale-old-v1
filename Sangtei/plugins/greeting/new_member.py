...
from Sangtei import USER_ID, LOG_CHANNEL, OWNER_ID, SUDO_USERS, SangteiCli
from Sangtei.database.federation_mongo import (get_fed_from_chat,
                                              get_fed_reason, is_user_fban)
from Sangtei.database.welcome_mongo import (DEFAUT_WELCOME, GetCaptchaSettings,
...
from Sangtei.helper.chat_status import isBotCan, isUserAdmin
from Sangtei.helper.welcome_helper.welcome_fillings import Welcomefillings
from Sangtei.helper.welcome_helper.welcome_send_message import \
    SendWelcomeMessage
from Sangtei.plugins.greeting.captcha import button_captcha, text_captcha


@SangteiCli.on_message(filters.new_chat_members)
async def NewMemeber(client, message):
    ...
    for NewUserJson in message.new_chat_members:
        ...
        # Sangtei's Special welcome for kami-samas!
        ...
        # Sangtei's Special welcome for her onii-chan gang!
        ...
        if is_user_fban(fed_id, user_id):
                ...
                if await isBotCan(message, permissions='can_restrict_members'):
                    if await SangteiCli.kick_chat_member(chat_id, user_id): 
                        text += '\nAction: `Banned`'
        ...
        if isWelcome(chat_id):
            ...
            WelcomeSentMessage = await SangteiCli.send_message(
                chat_id=chat_id,
                text=Text,
