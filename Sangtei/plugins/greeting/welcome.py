...
from Sangtei.helper.welcome_helper.welcome_send_message import SendWelcomeMessage
...
@SangteiCli.on_message(custom_filter.command(commands=('welcome')))
async def Welcome(client, message):
    ...
    if len(message.command) >= 2:
        ...
        if (
            ...
            await message.reply(
                "Member thar a piang ka lo lawm ziah tawh ang!",
                quote=True
            )
...
async def welcomeformat(message, chat_id, noformat=True):
    ...
    if isGetCaptcha(chat_id):
        ...
        if captcha_text == None:
            captcha_text = "Mihring i ni em ti chiang rawh"
