...
from Sangtei.database.welcome_mongo import SetCaptcha, isGetCaptcha
from Sangtei.helper import custom_filter
from Sangtei.helper.chat_status import isBotCan, isUserCan
from Sangtei.plugins.connection.connection import connection

CAPTCHA_WELCOME_TRUE = ['on', 'aw']
CAPTCHA_WELCOME_FALSE = ['off', 'aih']

@SangteiCli.on_message(custom_filter.command(commands=('captcha')))
async def Captcha(client, message):
    ...
    if (
        ...
        await message.reply(
            (
                f"{CaptchaSetting}\n\n"
                "To change this setting, try this command again followed by one of aw/aih/on/off"
            ),
            quote=True
        )  
