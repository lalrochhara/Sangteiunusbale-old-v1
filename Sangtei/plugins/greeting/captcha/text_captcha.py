...
from Sangtei.database.welcome_mongo import (AppendVerifiedUsers,
...
async def textCaptchaRedirect(message):
    ...
    if _match in [
        ...
        if new_user_id == user_id:
            ...
            CaptchaLoc = f"Sangtei/plugins/greeting/captcha/CaptchaDump/SangteiCaptcha_text_{new_user_id}_{new_chat_id}.png"
...
@SangteiCli.on_callback_query(filters.create(lambda _,__, query: 'textc_' in query.data))
async def textCaptchaCallBack(client: SangteiCli, callback_query: CallbackQuery):
    ...
    if (
        ...
        await SangteiCli.edit_message_caption(
            chat_id=user_id,
