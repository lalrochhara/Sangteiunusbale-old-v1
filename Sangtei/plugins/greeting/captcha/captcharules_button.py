...
from Sangtei.database.welcome_mongo import isRuleCaptcha
...
@SangteiCli.on_callback_query(filters.create(lambda _, __, query: 'captcharule_' in query.data))
async def captchaRules(client: SangteiCli, callback_query: CallbackQuery):
    ...
    passedButton = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(text='Group ah kir leh rawh', url=f'http://t.me/c/{str_chat_id}/{message_id}')
                ]
            ]
        )
