...
from Sangtei.database.report_mongo import get_report, reports_db
from Sangtei.helper import custom_filter
from Sangtei.helper.chat_status import isUserAdmin, isUserCan

REPORTS_TRUE = ['aw', 'on']
REPORTS_FALSE = ['aih', 'off']

@SangteiCli.on_message(custom_filter.command(commands=('reports')))
async def reports(client, message):
    ...
    if (
        ...
        if (
            ...
            await message.reply(
                "Users ten message an report thei tawh ang."
            )
