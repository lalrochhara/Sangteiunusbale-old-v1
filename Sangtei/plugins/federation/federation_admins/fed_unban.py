...
from Sangtei.database.federation_mongo import (get_connected_chats,
                                              get_fed_admins,
...
@SangteiCli.on_message(custom_filter.command(commands=('unfban')))
async def unfed_ban(client, message):
    ...
    get_user = await SangteiCli.get_users(
                user_ids=userID
            )
