...
from Sangtei.database.federation_mongo import (fed_promote,
...
@SangteiCli.on_message(custom_filter.command(commands=('fedpromote')))
async def FedPromote(client, message):
    ...
    user = await SangteiCli.get_users(
        user_ids=user_id
    )
...
@SangteiCli.on_callback_query(filters.create(lambda _, __, query: 'fedpromote_' in query.data))
async def fedpromote_callback(client: SangteiCli, callback_query: CallbackQuery):
    ...
    owner = await SangteiCli.get_users(
        user_ids=owner_id
    )
